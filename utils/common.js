export function filterByYear(data, year) {
  const filteredData = data.filter((item) => {
    ...
    return (
      itemYear === year &&
      new Date(item["Order Date"]) > new Date(`01-01-${year}`)
    );
  });

  return filteredData.sort(
    (a, b) => new Date(a["Order Date"]) - new Date(b["Order Date"])
  );
}
...
export function getSerializedData(orders, returns) {
  ...
  let data = orders.map((order) => ({
    ...order,
    daysToShip: Math.floor(
      (Date.parse(order["Ship Date"]) - Date.parse(order["Order Date"])) /
        86400000
    ),
    profitRatio:
      parseFloat(parseInt(order["Profit"]) / parseInt(order["Sales"])) * 100,
